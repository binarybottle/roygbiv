...
angular.module('visApp',[]).controller('visAppController',function($scope){
			...
			//'</p>';
				...
				var controls = addControls(camera)
				        ...
				        $.ajax({dataType: "json",
				           ...
				           success:function(data,textStatus,jqXHR){
				             ...
				             for (i=0;i<N.length;i++){
				               ...
				               loadMesh(data["filename"][i],scene,meshes)
				             }  
				           }})
				...
				window.addEventListener( 'resize', function(){onWindowResize(camera,renderer,controls)}, false );
				animate(controls,renderer,scene,camera);
				
				    $(document).click(function(e) {
				        if (e.shiftKey) {
				            //alert("shift+click")
				            objectPick(e,meshes,renderer,camera,scene)
				        } 
				    });
			...
			function loadVTK(name,scene,meshes){
			/* Loads VTK files
			...
			*/
			...
			//AK: WHY DOES THIS WORK BUT laodVTK DOESNT??
				function loadMesh(name,scene,meshes) {
			...
			var objectPick = function(e,meshes,renderer,camera,scene){
			        ...
			        picked_mesh.material.opacity = 1
			          ...
			          $scope.selectedLabel = picked_label_id
			          $scope.$apply()
			          selected_color = [picked_mesh.geometry.faces[0].color["r"], picked_mesh.geometry.faces[0].color["g"], picked_mesh.geometry.faces[0].color["b"]]
					  $scope.doBoxplots(picked_label_id, selected_color)
			...
			$scope.doBoxplots = function(labelID, color) {
